package history

import (
	"testing"
	"github.com/DexyProject/dexy-go/types"
	"fmt"
)

const (
	connection = "dummyurl"
	block = 4862998
)

var mgoConnection, _ = NewMongoHistory(connection)
var historyData = []types.Transaction{
	{
		TransactionID: "0x87012a0d870d47c3c93526c05c4a2f494054c3f4dd8584e94af7d8dd90a535f8",
		OrderHash: "0xEEAD6DBFC7340A56CAEDC044696A168870549A6A7F6F56961E84A54BD9970B8A",
		BlockNumber: 4862998,
		Timestamp: 1515233752,
		Taker: types.HexToAddress("0x997919a608788621dd48b3896f78dcda682fe91d"),
		Maker: types.HexToAddress("0x9f612fcb422d1971c1be7416c37e3ebc77c0de19"),
		Give: types.Trade{Token: types.HexToAddress("0x0000000000000000000000000000000000000000"), Amount: types.NewInt(30)},
		Get:  types.Trade{Token: types.HexToAddress("0xd26114cd6EE289AccF82350c8d8487fedB8A0C07"), Amount: types.NewInt(300)},
	},

	{
		TransactionID: "0x89012a0d870d47c3c93526c05c4a2f494054c3f4dd8584e94af7d8dd90a535f8",
		OrderHash: "0xEEAD6DBFC7340A56CAEDC044696A168870549A6A7F6F56961E84A54BD9970B8A",
		BlockNumber: 4862999,
		Timestamp: 1515233753,
		Taker: types.HexToAddress("0x997919a608788621dd48b3896f78dcda682fe91d"),
		Maker: types.HexToAddress("0x9f612fcb422d1971c1be7416c37e3ebc77c0de19"),
		Give: types.Trade{Token: types.HexToAddress("0x0000000000000000000000000000000000000000"), Amount: types.NewInt(30)},
		Get:  types.Trade{Token: types.HexToAddress("0xd26114cd6EE289AccF82350c8d8487fedB8A0C07"), Amount: types.NewInt(300)},
	},

	{
		TransactionID: "0x98012a0d870d47c3c93526c05c4a2f494054c3f4dd8584e94af7d8dd90a535f8",
		OrderHash: "0xEEAD6DBFC7340A56CAEDC044696A168870549A6A7F6F56961E84A54BD9970B8A",
		BlockNumber: 4862998,
		Timestamp: 1515233754,
		Taker: types.HexToAddress("0x997919a608788621dd48b3896f78dcda682fe91d"),
		Maker: types.HexToAddress("0x9f612fcb422d1971c1be7416c37e3ebc77c0de19"),
		Give: types.Trade{Token: types.HexToAddress("0x0000000000000000000000000000000000000000"), Amount: types.NewInt(30)},
		Get:  types.Trade{Token: types.HexToAddress("0xd26114cd6EE289AccF82350c8d8487fedB8A0C07"), Amount: types.NewInt(300)},
	},

}
func insertData(historyData []types.Transaction) (*MongoHistory, error) {
	mgoConnection.session.Clone()
	defer mgoConnection.session.Close()

	c := mgoConnection.session.DB(DBName).C(FileName)

	for _, tt := range historyData {
		err := c.Insert(tt)

		if err != nil {
			return nil, fmt.Errorf("could not add transaction history to database")
		}
	}
	return mgoConnection, nil
}

func TestMongoHistory_AggregateTransactions(t *testing.T) {
	insertData(historyData)
	response, err := (mgoConnection).AggregateTransactions(block)
	if response == nil {
		t.Errorf("Aggregation returned empty")
	}
	if err != nil {
		t.Errorf("Query not completed")
	}
}

